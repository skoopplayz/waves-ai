...
// Small helper: typewriter effect for the model label
...
try { typeWriterElement(modelSelected, modelDisplayNames[modelSourceValue], 20) } catch (e) { }
...
function getPreferredVoice() {
  ...
  let a = v.filter(x => x.lang && x.lang.startsWith("en") && /(asian|chinese|japanese|korean)/i.test(x.name) && /female/i.test(x.name))
  if (a.length) return a[0]
  let b = v.filter(x => x.lang && x.lang.startsWith("en") && /female/i.test(x.name))
  if (b.length) return b[0]
  let c = v.filter(x => x.lang && /^(zh|ja|ko|th|vi)/i.test(x.lang) && /female/i.test(x.name))
  if (c.length) return c[0]
  let d = v.filter(x => x.lang && x.lang.startsWith("en"))
...
function speakText(t, voice, rate, pitch, onEnd) {
  ...
  function next() {
    if (idx < chunks.length) {
      ...
      let ft = setTimeout(() => { u.onend = null; idx++; next() }, 25000)
      u.onend = () => { clearTimeout(ft); idx++; next() }
...
function typeWriterEffect(msg, msgType, skippable = true, callback) {
  isTyping = true
  sendMsg.innerHTML = '<i aria-label="Enter Button" class="fas fa-stop"></i>'
  const d = document.createElement("div")
  ...
  function completeTyping() {
    ...
    reHighlight()
    chatBody.scrollTo({ top: chatBody.scrollHeight, behavior: "smooth" })
    sendMsg.innerHTML = '<i aria-label="Enter Button" class="fas fa-arrow-up"></i>'
    isTyping = false
    ...
    if (msgType === "ai") {
      ...
      const cp = document.createElement("button"); cp.classList.add("ai-button"); cp.innerHTML = '<i class="fa-regular fa-copy"> Copy Response</i>'
      ...
      bc.appendChild(cp)
      const ra = document.createElement("button"); ra.classList.add("ai-button"); ra.innerHTML = '<i class="fa-solid fa-volume-up"> Speak Response To Me</i>'; ra.dataset.speaking = "false"
      ...
      bc.appendChild(ra)
      const rg = document.createElement("button"); rg.classList.add("ai-button"); rg.innerHTML = '<i class="fa-solid fa-arrows-rotate"> ReGenerate Response</i>'
  ...
  if (/<[a-z][\s\S]*>/i.test(msg)) {
    ...
    function nextToken() {
      ...
      if (tk.startsWith("<")) {
        ...
        function typeC() {
          if (finished) return
          if (ci < tk.length) { out += tk.charAt(ci); txt.innerHTML = out; reHighlight(); ci++; chatBody.scrollTop = chatBody.scrollHeight; timers.push(setTimeout(typeC, speed)) }
          else { ti++; nextToken() }
        }
        typeC()
      }
...
sendMsg.addEventListener("click", () => {
  ...
  if (isFetching) { if (abortController) { abortController.abort(); abortController = null } isFetching = false; sendMsg.innerHTML = '<i aria-label="Enter Button" class="fas fa-arrow-up"></i>'; document.querySelectorAll(".thinking-indicator").forEach(x => x.remove()); return }
  ...
  abortController = new AbortController(); isFetching = true; sendMsg.innerHTML = '<i aria-label="Enter Button" class="fas fa-stop"></i>'; if (window.NProgress) NProgress.start()

  // Build payload for groq/openai endpoint
  const payload = {
    model: modelSourceValue,
    messages: [
      { role: "system", content: "Your name is TutorMate AI! Your primary role is to assist students with their work or topic of discussion by providing supportive guidance. It’s essential that you remain helpful, concise, and provide examples and practice problems when appropriate. Avoid unsafe or illicit instructions. Be polite and educational." },
  ...
  // Attempt to use next API key from the list
  let apiKey
  try { apiKey = getNextApiKey() } catch (err) { isFetching = false; showToast('No API keys available. Add keys to localStorage or ensure API_KEYS_URL is reachable.', 'error'); aiInput.disabled = false; sendMsg.innerHTML = '<i aria-label="Enter Button" class="fas fa-arrow-up"></i>'; if (window.NProgress) NProgress.done(); return }
  ...
  fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    isFetching = false; document.querySelectorAll(".thinking-indicator").forEach(x => x.remove()); if (window.NProgress) NProgress.done()
    const aiResponse = data.choices?.[0]?.message?.content || data.choices?.[0]?.text || "No response from AI."
    const formatted = formatAIResponse(aiResponse)
    const cleaned = cleanupMessage(formatted)
    typeWriterEffect(cleaned, "ai")
    messageHistory.push({ role: "assistant", content: aiResponse })
  }).catch(err => {
    isFetching = false; if (window.NProgress) NProgress.done()
    if (err.name !== "AbortError") showToast("Error communicating with AI.", "error")
    aiInput.disabled = false; sendMsg.innerHTML = '<i aria-label="Enter Button" class="fas fa-arrow-up"></i>'
    console.error(err)
  })
})
...
function regenerateResponse(regenPrompt, oldMessage, attempt = 0) {
  ...
  chatBody.appendChild(ti); chatBody.scrollTo({ top: chatBody.scrollHeight, behavior: "smooth" })
  abortController = new AbortController(); isFetching = true; sendMsg.innerHTML = '<i aria-label="Enter Button" class="fas fa-stop"></i>'; if (window.NProgress) NProgress.start()
  let msgs = [{ role: "system", content: "You are a highly advanced, deeply trained, and exceptionally intelligent AI. Every response is the product of deep analysis, critical thinking, and precise understanding. Provide clear answers, examples, and when appropriate short practice problems. Do not provide unsafe or disallowed content." }]
  ...
  let apiKey
  try { apiKey = getNextApiKey() } catch (err) { isFetching = false; showToast('No API keys available for regeneration.', 'error'); if (window.NProgress) NProgress.done(); return }
